...
exports.login = catchAsync(async (req, res, next) => {
  ...
  if (!email || !password)
    return next(new AppErrore('Please provide email and password!!', 400));

  const filterdUser = await User.findOne({ email }).select('+password');
  const correct = await filterdUser.correctPassword(
    password,
    filterdUser.password
  );
  if (!filterdUser || !correct)
    return next(new AppErrore('Incorrect email or password', 401));
...
exports.protact = catchAsync(async (req, res, next) => {
  ...
  if (!token)
    return next(
      new AppErrore('You are not logged in!! please log in to get Access.', 401)
    );
  ...
  if (!freshUser)
    return next(
      new AppErrore(
        'The user bloging to this token does no longer exist. ',
        401
      )
    );
  ...
  if (freshUser.changedPasswordAfter(decoded.iat)) {
    return next(
      new AppErrore('User recently changed password! please log in again.', 401)
    );
  }
...
exports.restrictTo = (...roles) => {
  return (req, res, next) => {
    if (!roles.includes(req.user.role)) {
      return next(
        new AppErrore('You do not have permission to perform this action', 403)
      );
    }
    next();
  };
...
exports.isLoggedin = catchAsync(async (req, res, next) => {
  ...
  if (req.cookies.jwt) {
    ...
    res.locals.user = null; // Ensure user is set to null if no JWT
  }
...
exports.resetPassword = catchAsync(async (req, res, next) => {
  ...
  if (!currentUser)
    return next(new AppErrore('The token is invalied OR has expired', 400));
